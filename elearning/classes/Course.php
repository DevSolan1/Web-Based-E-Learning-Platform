...
class Course {
    ...
    public function create($data) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO courses (teacher_id, category_id, title, slug, description, short_description, price, level, thumbnail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) return ['success' => false, 'message' => 'Database error'];
        
        $stmt->bind_param("iissssdss", $teacherId, $categoryId, $title, $slug, $description, $shortDesc, $price, $level, $thumbnail);
