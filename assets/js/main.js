...
function updateVideoProgress(currentTime) {
    ...
    const basePath = window.location.pathname.includes('/student/') ? '../' : '';
    
    fetch(basePath + 'ajax/update_progress.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ video_id: parseInt(videoId), watched_duration: currentTime })
    });
}
...
function markVideoComplete() {
    ...
    fetch(basePath + 'ajax/update_progress.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ video_id: parseInt(videoId), is_completed: true })
    }).then(() => {
        const playlistItem = document.querySelector(`[data-video-id="${videoId}"]`);
        if (playlistItem) {
            playlistItem.classList.add('completed');
            const icon = playlistItem.querySelector('i');
            if (icon) {
...
function checkCourseCompletion() {
    ...
    fetch(basePath + `ajax/check_completion.php?course_id=${courseId}`)
        .then(res => res.json())
        .then(data => {
            if (data.completed) {
                showCompletionModal();
