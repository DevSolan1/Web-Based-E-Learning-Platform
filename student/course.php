        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                <div class="video-container mb-3">
                    <video id="courseVideo" controls data-video-id="<?= $currentVideo['id'] ?>" data-course-id="<?= $courseId ?>" style="width:100%; max-height:500px; background:#000;">
                        <source src="../uploads/videos/<?= $currentVideo['video_url'] ?>" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>
                </div>
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="mb-0"><?= htmlspecialchars($currentVideo['title']) ?></h4>
                    <?php 
                    $currentProgress = $video->getProgress($_SESSION['user_id'], $currentVideo['id']);
                    $isCurrentComplete = $currentProgress && $currentProgress['is_completed'];
                    ?>
                    <?php if (!$isCurrentComplete): ?>
                    <button id="markCompleteBtn" class="btn btn-success" onclick="markVideoCompleteManual()">
                        <i class="bi bi-check-circle me-1"></i>Mark as Complete
                    </button>
                    <?php else: ?>
                    <span class="badge bg-success fs-6 p-2"><i class="bi bi-check-circle me-1"></i>Completed</span>
                    <?php endif; ?>
                </div>
...
<script>
function markVideoCompleteManual() {
    ...
    fetch('../ajax/update_progress.php', {
        ...
        body: JSON.stringify({ video_id: parseInt(videoId), is_completed: true, watched_duration: 0 })
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
